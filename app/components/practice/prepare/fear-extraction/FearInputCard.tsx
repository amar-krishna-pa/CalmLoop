...
import type { FearToSave } from "@/app/lib/zod/save-fears-schema";
...
export default function FearInputCard({ onSaved }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { fears }: { fears: ExtractedFearPreview[] } =
        await response.json();
  ...
  async function handleSave({ fears }: { fears: FearToSave[] }) {
    if (isSaving) return;
    ...
    <>
      ...
      {previewFears && (
        <ExtractionPreviewModal
          ...
          isSaving={isSaving}
          onDiscard={() => {
            if (!isSaving) setPreviewFears(null);
          }}
          onSave={handleSave}
        />
      )}
